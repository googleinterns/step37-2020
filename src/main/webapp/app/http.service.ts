...
import { defaultIsTest } from '../utils';
...
export class HttpService {
  ...
  getProjectGraphData(id: string): Promise<ProjectGraphData> {
    if (defaultIsTest) {
      return new Promise(resolve => resolve(this.fakeService.getProjectGraphData(id)));
    }
    return this.http.get<ProjectGraphData>(`/get-project-data?id="${id}"`).toPromise();
  }
